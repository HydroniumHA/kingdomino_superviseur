...
# Varibale globale pour les informations de la nouvelle tuile
nouvelle_tuile = {
    "area": 0,
    "coordinates": None,
    "last_detected_time": None  # Temps de la dernière détection
...
def isolateTile(img, contours):
    ...
    for cnt in contours:
        ...
        if nouvelle_tuile_area > chateau['area'] * 1.75:
            print("Nouvelle tuile potentielle détectée avec aire :", nouvelle_tuile_area, " (Château aire :", chateau['area'], ")")
            ...
            if contours_tuile:
                ...
                if nouvelle_tuile_w > 0 and nouvelle_tuile_h > 0:
                    ...
                    if nouvelle_tuile["last_detected_time"] is None:
                        nouvelle_tuile["last_detected_time"] = time.time()
                        print("Nouvelle tuile détectée, en attente de confirmation...")
                    elif time.time() - nouvelle_tuile["last_detected_time"] >= TILE_CONFIRMATION_TIME:
                        # Afficher
...
def processTile(cropped_img, area):
    ...
    nouvelle_tuile["last_detected_time"] = None
